import { Component, OnInit, afterNextRender, afterRender, ElementRef, ViewChild, OnDestroy, inject, ChangeDetectionStrategy, ChangeDetectorRef } from '@angular/core';
...
export class CartsComponent implements OnInit, OnDestroy {
  ...
  ngOnDestroy(): void {
    ...
    if (this.cartsTable) {
      this.lastScrollPosition = this.cartsTable.nativeElement.scrollTop;
    }
  ...
  private initializeCartsTable(): void {
    ...
    cartRows.forEach(row => {
      ...
      // Add handler for selected rows
      row.addEventListener('click', (event) => {
        // Avoid selection when clicking buttons
        if (!(event.target as HTMLElement).closest('button')) {
          const email = row.getAttribute('data-email');
          if (email) {
            this.navigateToCartDetails(email);
          }
        }
      });
    });
  ...
  private setupTableResizeObserver(): void {
    ...
    this.resizeObserver = new ResizeObserver(entries => {
      entries.forEach(entry => {
        console.log('The cart table has changed size:', entry.contentRect);
  ...
  private updateTableVisuals(): void {
    ...
    cartRows.forEach((row, index) => {
      ...
      // Highlight disabled carts
      const statusCell = row.querySelector('.cart-status');
      if (statusCell) {
        if (statusCell.textContent?.toLowerCase().includes('disabled')) {
          row.classList.add('disabled-cart');
        } else {
          row.classList.remove('disabled-cart');
        }
      }
    });
  }
  ...
  private restoreScrollPosition(): void {
    if (this.lastScrollPosition > 0 && this.cartsTable) {
      this.cartsTable.nativeElement.scrollTop = this.lastScrollPosition;
    }
  }
