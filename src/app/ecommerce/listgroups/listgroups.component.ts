import { Component, OnInit, ViewChild, ElementRef, afterNextRender, afterRender, inject, DestroyRef, ChangeDetectionStrategy, ChangeDetectorRef } from '@angular/core';
...
export class ListgroupsComponent implements OnInit {
  ...
  private setupResizeObserver(): void {
    ...
    this.resizeObserver = new ResizeObserver(entries => {
      entries.forEach(entry => {
        console.log('The group container has been resized:', entry.contentRect);
  ...
  private restoreScrollPosition(): void {
    if (this.lastScrollPosition > 0 && this.groupsContainer) {
      this.groupsContainer.nativeElement.scrollTop = this.lastScrollPosition;
    }
  }
  ...
  loadRecords(idGroup: string): void {
    ...
    if (this.groupsContainer) {
      this.lastScrollPosition = this.groupsContainer.nativeElement.scrollTop;
    }
