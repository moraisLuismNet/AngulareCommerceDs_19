...
export class RecordsComponent implements OnInit {
  ...
  private initializeTable(): void {
    // Initialize table with any custom logic
  }

  private setupTableResizeObserver(): void {
    // Set up resize observer for the table
    const tableElement = this.recordsTable?.nativeElement;
    if (tableElement) {
  ...
  private saveScrollPosition(): void {
    if (this.recordsTable?.nativeElement) {
      this.lastScrollPosition = this.recordsTable.nativeElement.scrollTop;
    }
  }

  private restoreScrollPosition(): void {
    if (this.recordsTable?.nativeElement && this.lastScrollPosition > 0) {
      this.recordsTable.nativeElement.scrollTop = this.lastScrollPosition;
    }
  }
